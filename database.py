    ...
    if row is None:
        ...
        await db.execute("""
            ...
            SET username = ?, is_blocked = 0, дата_входа = ?,
                ...
                traffic_source = CASE WHEN traffic_source = '' OR traffic_source IS NULL THEN ? ELSE traffic_source END,
                status = 'active',
                pressure_started_at = NULL,
...
async def get_inactive_leads_for_pressure() -> list[int]:
    """Retrieves list of user IDs eligible for pressure funnel: stuck in stage 2 or 3 for >24 hours."""
    db = get_db()
    async with db.execute("""
        ...
        WHERE is_blocked = 0 
          ...
          AND (
              status = 'cold' 
              OR (этап_воронки IN (2, 3) AND (julianday('now') - julianday(дата_входа)) >= 1.0)
          )
    """) as cursor:
        rows = await cursor.fetchall()
        return [row[0] for row in rows]
