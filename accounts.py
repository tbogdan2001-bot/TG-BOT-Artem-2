...
async def join_target_channel_by_invite(client: TelegramClient):
    """
    Checks if the userbot is in TARGET_CHANNEL and joins if not.
    """
    target = config.TARGET_CHANNEL
    if not target:
        logger.warning("TARGET_CHANNEL is not set. Skipping self-join check.")
        return
        
    logger.info(f"Checking if userbot needs to join target channel: {target}")
    
    # 1. Parse invite hash if it's an invite link
    invite_hash = None
    if "t.me/+" in target:
        invite_hash = target.split("t.me/+/")[1].strip() if "t.me/+/" in target else target.split("t.me/+")[1].strip()
    elif "t.me/joinchat/" in target:
        invite_hash = target.split("t.me/joinchat/")[1].strip()
        
    if invite_hash:
        # Strip any slash or query parameters
        if "/" in invite_hash:
        ...
        try:
            ...
            logger.info(f"Successfully joined target channel via invite link: {target}")
        except UserAlreadyParticipantError:
            logger.info(f"Userbot is already a participant of target channel: {target}")
        except Exception as e:
            logger.error(f"Failed to join target channel {target} via invite link: {e}")
    else:
        # Try as ID or username
        try:
            channel_entity = target
            if isinstance(target, str) and (target.startswith("-") or target.isdigit()):
                channel_entity = int(target)
            await client.get_entity(channel_entity)
            logger.info(f"Userbot already has access to target channel: {target}")
        except Exception:
            try:
                from telethon.tl.functions.channels import JoinChannelRequest
                await client(JoinChannelRequest(channel_entity))
                logger.info(f"Successfully joined target channel: {target}")
            except Exception as join_err:
                logger.error(f"Failed to join target channel {target}: {join_err}")
