...
async def notify_closer_hub(bot: Bot, user_id: int):
    ...
    # CHANGED: Added and translated Q1, Q2, Q3 quiz answers to beautiful Russian text
    q1_raw = user.get("quiz_q1") or "Не указан"
    q2_raw = user.get("quiz_q2") or "Не указан"
    q3_raw = user.get("quiz_q3") or "Не указан"
    ...
    q2_map = {
        ...
        "active_trading": "📊 Активный трейдинг",
        "learning": "🎓 Обучение"
    }
    q3_map = {
        "under_500": "💵 До $500",
        "between_500_5000": "💳 $500 - $5000",
        "above_5000": "💎 От $5000"
    }
    
    q1 = q1_map.get(q1_raw, q1_raw)
    q2 = q2_map.get(q2_raw, q2_raw)
    q3 = q3_map.get(q3_raw, q3_raw)
