...
# Check if Redis is actually reachable before initializing storage to prevent crashes
use_redis = False
if config.REDIS_URL:
    import socket
    from urllib.parse import urlparse
    logger.info(f"Testing connection to Redis FSM storage: {config.REDIS_URL}")
    try:
        parsed = urlparse(config.REDIS_URL)
        host = parsed.hostname or "localhost"
        port = parsed.port or 6379
        # 1-second timeout socket test
        with socket.create_connection((host, port), timeout=1.0):
            use_redis = True
    except Exception as socket_err:
        logger.warning(f"Redis is unreachable: {socket_err}. Falling back to MemoryStorage.")
...
if use_redis:
    ...
    logger.warning("FSM will use MemoryStorage (non-persistent).")
